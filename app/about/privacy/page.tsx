...
const pageStyles: CSSProperties = {
    ...
    color: "#0a0a0a",
    overflowX: "hidden", // Prevents unintended side scrolling
    width: "100%"
};
...
const containerStyles: CSSProperties = {
    maxWidth: 1280,
    margin: "0 auto",
    paddingLeft: "min(5%, 40px)", // Scales gutters downward smoothly on narrow mobile screens
    ...
    return (
        <section 
            style={{ 
                ...
                borderRadius: 24, 
                padding: "min(28px, 6%)", // Fluid inside margins
        ...
        >
            <h2 style={{ ...headingFont, fontWeight: 600, fontSize: "min(30px, 7.5vw)", lineHeight: 1.1, letterSpacing: "-0.02em", color: "#0a0a0a", margin: 0 }}>{title}</h2>
            {copy ? (
                <p style={{ ...bodyFont, fontSize: 16, lineHeight: 1.7, color: "#4f5560", margin: "16px 0 0", textWrap: "pretty" }}>{copy}</p>
            ) : null}
            {items ? (
                <ul style={{ margin: "16px 0 0", paddingLeft: 20, display: "grid", gap: 10, width: "100%", boxSizing: "border-box" }}>
                    {items.map((item) => (
                        <li key={item} style={{ ...bodyFont, fontSize: 16, lineHeight: 1.7, color: "#28313b", wordBreak: "break-word" }}>
                            {item}
                        </li>
                    ))}
                </ul>
            ...
            {note ? (
                <p style={{ ...bodyFont, fontSize: 14, lineHeight: 1.7, color: "#66707a", margin: "14px 0 0", textWrap: "pretty" }}>{note}</p>
            ) : null}
        </section>
    );
}
...
export default function Page() {
    return (
        <div style={pageStyles}>
            ...
            <main id="top" style={{ position: "relative", width: "100%" }}>
                {/* HERO AND QUICK SUMMARY PANEL */}
                <section style={{ padding: "180px min(5%, 40px) 88px", boxSizing: "border-box", width: "100%" }}>
                    <div style={containerStyles}>
                        <div 
                            style={{ 
                                display: "grid", 
                                // Dynamic auto-fit turns side-by-side columns into stacked rows on smaller mobile screens
                        ...
                        >
                            <div style={{ minWidth: 0 }}>
                                ...
                                <h1 style={{ ...headingFont, fontWeight: 500, fontSize: "clamp(2.5rem, 6vw, 5.5rem)", lineHeight: 0.98, letterSpacing: "-0.04em", color: "#0a0a0a", margin: "18px 0 0", textWrap: "balance" }}>How NevUp handles your data.</h1>
                                <p style={{ ...bodyFont, fontSize: "min(19px, 5vw)", lineHeight: 1.6, color: "#4f5560", margin: "24px 0 0", maxWidth: 760, textWrap: "pretty" }}>
                                    Effective Date: June 2026 | Last Updated: June 2026 | www.nevup.in
                                </p>
                                <p style={{ ...bodyFont, fontSize: 16, lineHeight: 1.75, color: "#4f5560", margin: "18px 0 0", maxWidth: 840, textWrap: "pretty" }}>
                            ...
                            {/* QUICK SUMMARY CARD */}
                            <div style={{ background: "#0a0a0a", color: "#fffaf2", borderRadius: 24, padding: "min(24px, 5%)", boxShadow: "0 30px 80px rgba(0,0,0,0.18)", boxSizing: "border-box", width: "100%", minWidth: 0 }}>
                                ...
                                <div style={{ display: "grid", gap: 14, marginTop: 18 }}>
                                    {summaryItems.map((item) => (
                                        <div key={item.title} style={{ padding: 16, borderRadius: 16, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,250,242,0.08)", minWidth: 0 }}>
                                            <p style={{ ...headingFont, fontWeight: 600, fontSize: 18, margin: 0, color: "#fffaf2" }}>{item.title}</p>
                                            <p style={{ ...bodyFont, fontSize: 13, lineHeight: 1.6, margin: "8px 0 0", color: "rgba(255,250,242,0.72)", textWrap: "pretty" }}>{item.copy}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                ...
                {/* POLICY CARDS & NAVIGATION JUMP LINKS */}
                <section style={{ padding: "0 min(5%, 40px) 96px", boxSizing: "border-box", width: "100%" }}>
                    <div style={{ ...containerStyles, display: "grid", gap: 18 }}>
                        {/* NAVIGATION PILL LINKS BAR */}
                        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px 10px", marginBottom: 12, width: "100%" }}>
                            {policySections.map((section) => (
                                <a key={section.id} href={`#${section.id}`} style={{ ...bodyFont, fontSize: 13, textDecoration: "none", color: "#0a0a0a", border: "1px solid rgba(10,10,10,0.12)", background: "rgba(255,255,255,0.7)", borderRadius: 999, padding: "10px 14px", display: "inline-block" }}>
                                    {section.title}
                                </a>
                            ))}
                        </div>
                        ...
                        {policySections.map((section) => (
                            <div key={section.id} id={section.id} style={{ scrollMarginTop: 120, width: "100%" }}>
